...
class RoutingSolverUPS(cvrptw_routing_solver.RoutingSolverCVRPTW):
    """
    solver specific to CVRP
    """
    def __init__(self,manager_stop, config_cst):
        super(RoutingSolverUPS,self).__init__(manager_stop,config_cst)
    ...
    def solve_routing(self):
        ...
        input_file =open(useful_paths.FILE_TO_INPUT_SOLVER_UPS, 'w')
    ...
    def execute_command_solver():
        ...
        :return:
        """
        if os.path.exists(useful_paths.FILE_TO_OUTPUT_SOLVER_UPS):
            os.remove(useful_paths.FILE_TO_OUTPUT_SOLVER_UPS)
        comp = 0
        while not os.path.exists(useful_paths.FILE_TO_OUTPUT_SOLVER_UPS):
            path_use = os.path.join('..','..','solver','ups','ups.lsp')
            instance_name = os.path.join('..','..','solver','ups','instances','input_cvrptw.txt')
            results_name = os.path.join('..','..','solver','ups','results_routing.txt')
            output_name = os.path.join('..','..','solver','ups','output.txt')
            myCmd = 'localsolver ' + path_use + '  inFileName=' +instance_name +' solFileName=' + results_name +' lsTimeLimit=30 > ' + output_name
    ...
    def parse_results(map_new_old_id):
        ...
        """
        results_file = open(useful_paths.FILE_TO_OUTPUT_SOLVER_UPS, 'r')
    ...
    def store_results_in_csv(self,filename,map_new_old_id):
        """
        Store the routing results in filename, under the form of an exploitable file
